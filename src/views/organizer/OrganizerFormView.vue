...
function saveOrganizer() {
  OrganizerService.saveOrganizer(organizer.value)
    .then((response) => {
      store.updateMessage('You are successfully add a new organizer: ' + response.data.organization)
      setTimeout(() => {
        store.resetMessage()
      }, 3000)
...
<template>
  <div>
    ...
    <form @submit.prevent="saveOrganizer">
      ...
      <label class="block text-gray-500 font-bold">Organization</label>
      <input v-model="organizer.organization" type="text" placeholder="Organization" class="h-13 w-1/4 px-2.5 text-xl border border-gray-400 focus:border-emerald-500 focus:outline-none mb-6" />
